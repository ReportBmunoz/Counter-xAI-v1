...
class MultiResolutionDataset(Dataset):
    ...
    # def __init__(self, path, transform, resolution=128, labels=False, filter_label=None):
        self.env = lmdb.open(
            path,
            max_readers=32,
            readonly=True,
    ...
    def __getitem__(self, index):
        with self.env.begin(write=False) as txn:
            ...
            if self.labels:
                ...
                # label_key = f"256-{str(index).zfill(5)}-label".encode("utf-8")
        ...
        # Ajustar el formato de la etiqueta, eliminando el intento de indexación
        if self.filter_label is not None:
...
#     no_finding_label_idx = complete_dataset.PRED_LABEL.index("0")
...
def label_vs_no_finding_dataset(args, transform, return_labels):
    ...
    # En PCAM, solo hay dos etiquetas: 0 y 1
    no_finding_label_idx = 0 # Etiqueta 0 en PCAM indica "No Finding"
    filter_label_idx = 1  # Etiqueta 1 en PCAM indica "Finding"

    # get the index of no_finding samples and also filter label respectively
    no_finding_sample_index = np.where(targets == no_finding_label_idx)[0]
    filter_sample_index = np.where(targets == filter_label_idx)[0]

    # Limitamos la cantidad de muestras según el tamaño mínimo de ambas clases
    min_length = min(len(no_finding_sample_index), len(filter_sample_index))
    no_finding_sample_index = no_finding_sample_index[:min_length]
    filter_sample_index = filter_sample_index[:min_length]
